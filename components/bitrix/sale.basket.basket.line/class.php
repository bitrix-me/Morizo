		...
		foreach ($arBasketItems as &$arItem)
		{
			...
			if (array_key_exists($arItem["PRODUCT_ID"], $arSku2Parent)) // if sku element doesn't have value of some property - we'll show parent element value instead
			{
				foreach ($arImgFields as $field) // fields to be filled with parents' values if empty
				{
					...
					## Mike - check field name
					if(in_array($field, $arImgFields)){

					    if(isset($arProductData[$parentId][$fieldVal])){

                            $fieldVal =  $field;

                        }elseif(isset($arProductData[$parentId][$fieldVal."_VALUE"])){
			...
			## Mike - check article image file
			if(strlen($arItem["PROPERTY_CML2_ARTICLE_VALUE"]) && file_exists($_SERVER["DOCUMENT_ROOT"]."/upload/photo/".$arItem["PROPERTY_CML2_ARTICLE_VALUE"].".jpg")){

                $arImage = CFile::MakeFileArray($_SERVER["DOCUMENT_ROOT"]."/upload/photo/".$arItem["PROPERTY_CML2_ARTICLE_VALUE"].".jpg");

                if ($arImage)
                {
                    $cacheImageFile = substr($arImage["tmp_name"], strlen($_SERVER["DOCUMENT_ROOT"]));

                    $arItem["PICTURE_SRC"] = $cacheImageFile;

                    ## Mike - ресайзинг если нужно, в задаче этого нет
                    /*
                    CFile::ResizeImage(
                        $arImage,
            ...
            }elseif (isset($arItem["PREVIEW_PICTURE"]) && intval($arItem["PREVIEW_PICTURE"]) > 0)
